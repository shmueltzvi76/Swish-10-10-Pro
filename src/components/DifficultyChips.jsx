...
export default function DifficultyChips({ selected, onToggle, compact = false, grid = false }) {
  return (
    <div className={grid ? 'grid grid-cols-2 gap-2' : (compact ? 'flex flex-wrap gap-1.5' : 'flex flex-wrap gap-2')}>
      {DIFFICULTY_MODIFIERS.map(mod => {
        ...
        return (
          ...
          >
            {!compact && <Dumbbell size={11} className={`shrink-0 ${isActive ? 'text-black' : 'text-[#FF8A00]'}`} />}
            {mod.label}
          </button>
        );
      })}
